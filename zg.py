...
class ZipGobblerBot:
    ...
    async def cmd_start(self, update: Update, context: CallbackContext):
        ...
        telethon_status = "✅ Active" if self.telethon and self.telethon.connected else "❌ Inactive"
    ...
    async def cmd_compress(self, update: Update, context: CallbackContext):
        ...
        await update.message.reply_text(
            ...
            f"⚡ Telethon: {'✅' if self.telethon and self.telethon.connected else '❌'}\n\n"
    ...
    async def _handle_extract(self, update: Update, context: CallbackContext, document):
        ...
        if document.file_size > MAX_FILE_SIZE:
            if self.telethon and self.telethon.connected:
                use_telethon = True
                await update.message.reply_text(
                    f"⚡ *File > 2GB detected!*\n"
    ...
    async def _process_extract(self, update: Update, job: ExtractJob, document):
        ...
        try:
            ...
            if job.use_telethon and self.telethon:
                await update.message.reply_text(
                    "⚡ *Downloading via Telethon...*\n"
                    "No size limit!",
                    parse_mode='Markdown'
                )
    ...
    def run(self):
        ...
        try:
            ...
            if self.telethon:
                try:
                    asyncio.get_event_loop().run_until_complete(self.telethon.close())
                except RuntimeError:
                    asyncio.run(self.telethon.close())
